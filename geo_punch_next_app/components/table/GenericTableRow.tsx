...
const GenericTableRow = <T extends FieldValues>(props: TableRowProps<T>) => {
    const { rowData, columns, editURL, deleteFunction, allowDelete, allowEdit, control } = props;
    return (
        <TableRow>
                ...
                <TableCell key={colIndex} fixedLength={col.type !== 'action'}>
                    {(() => { 
                        switch ((col.type ?? '').toLowerCase()) {
                            ...
                            case 'datetime':
                                return <p>{formatDateTime(rowData[col.key] as Date)}</p>;
                            ...
                            case 'action':
                                return (allowEdit || allowDelete) ? (
                                    <div className="flex flex-row justify-between items-center h-full text-[0.9rem] border-gray/20 w-fit max-w-20 p-2">
                                        {(!!allowEdit) && (
